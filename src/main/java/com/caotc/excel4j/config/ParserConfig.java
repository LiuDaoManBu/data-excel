...
public class ParserConfig {
  ...
  public <T> Optional<ConstructType> getConstructType(Class<T> type) {
    return getConstructType(TypeToken.of(type));
  }

  public <T> Optional<ConstructType> getConstructType(TypeToken<T> token) {
    if(token.isArray()) {
      return Optional.of(ConstructType.ITERABLE);
    }
    // TODO 父子关系生效?
    ...
    return optional.isPresent() ? optional : Optional.of(ConstructType.OBJECT);
  }
